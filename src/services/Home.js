...
export const filteredProductIds = ["SG003","CG002","CG004","AU002","AU003"];
export const filteredProducts = products.filter(p => filteredProductIds.includes(p.id));

export const valorantProducts = products.filter(p => p.juego === 'Valorant').slice(0, 5);
export const freeShippingProducts = products.filter(p => p.envioGratis).slice(0, 5);
